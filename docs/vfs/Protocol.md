...
Used to receiveq a **cursor** to a root of fs tree.  
...
Returned handle is guaranteed to be unique and valid untile the file is closed (unless server explodes).
...
Given a *writable* **handle** writes data to the file.
...
Given a *writable* **handle** updates metadata of the file.
